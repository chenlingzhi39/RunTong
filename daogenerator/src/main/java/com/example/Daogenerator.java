...
public class Daogenerator {
    ...
    public static void main(String[] args) throws Exception {
        int version = 1;
        String defaultJavaPackage = "de.greenrobot.dao";
        Schema schema = new Schema(version, defaultJavaPackage);
        //addTable(schema);
        addMark(schema);
        // /表示根目录， ./表示当前路径， ../表示上一级父目录
        new DaoGenerator().generateAll(schema, "./callba/src/main/java");
    }
private static void addMark(Schema schema){
    Entity item = schema.addEntity("Mark");
    item.addIdProperty();
    item.addStringProperty("username").notNull();
    item.addDateProperty("date").notNull();
    item.addIntProperty("month").notNull();
}

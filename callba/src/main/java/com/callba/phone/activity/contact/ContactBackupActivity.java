...
@ActivityFragmentInject(
		contentViewId = R.layout.contact_backup
)
public class ContactBackupActivity extends BaseActivity implements
		OnClickListener {
	...
	private void restoreContacts(List<ContactPersonEntity> remoteContacts) {
		ContactBackupActivity.this.getContentResolver().delete(
		...
		for (int i = 0; i < remoteContacts.size(); i++) {
			...
			Uri rawContactUri = ContactBackupActivity.this.getContentResolver()
			...
			values.put(StructuredName.GIVEN_NAME, remoteContacts.get(i)
					.getDisplayName());
			ContactBackupActivity.this.getContentResolver().insert(
